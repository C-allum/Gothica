    ...
    try:
        ...
        if message.guild.id != 828411760365142076 and message.author != client.user:
            ...
            #Character Edit Subroutine
                ...
                history = await client.get_channel(indexchannel).history(limit=2000, oldest_first=True).flatten()
            ...
            #Verify Command
                ...
                if vermem[0] != []:
                    ...
                    if rand == 1:
                        ...
                        await client.get_channel(828411760847356005).send(embed = discord.Embed(title = "Say hello to " + str(vername) + "!", description = "Hello everyone! We were taking detailed notes about the xio colony in the lower halls and found a new visitor! Please say hello to " + str(verping) + "!\nNow if you'll excuse us, we must go back to find out precisely how quickly those broodmothers spawn.", colour = embcol))

                    elif rand == 2:

                        await client.get_channel(828411760847356005).send(embed = discord.Embed(title = "Look - we found someone new - " + str(vername) + "!", description = "Pardon me. We were helping Sophie care for a sick tentacle, and it spat up a person! Would one of you please take care of " + str(verping) + " while We help Sophie clean up the excess slime?", colour = embcol))

                    elif rand == 3:

                        await client.get_channel(828411760847356005).send(embed = discord.Embed(title = "Can someone look after " + str(vername) + "?", description = str(verping) + ", here, got caught trying to look under Our skirts. Apparently, they have never heard what happens when you stare into the Abyss because they seem to be stunned by what was down there. We're sure a hot meal will do the trick though.", colour = embcol))

                    elif rand == 4:

                        await client.get_channel(828411760847356005).send(embed = discord.Embed(title = str(vername) + " just turned up!", description = "We were mucking out the Cathedral's prison cells and found " + str(verping) + " tied to a post, promising to be good. Come say hello to the newest lewd convert!", colour = embcol))

                    elif rand == 5:

                        await client.get_channel(828411760847356005).send(embed = discord.Embed(title = "We caught " + str(vername) + " trying to sneak in!", description = str(verping) + " thought they could get in without us noticing. Everybody, make sure they feel welcome!", colour = embcol))

                    elif rand == 6:

                        await client.get_channel(828411760847356005).send(embed = discord.Embed(title = str(vername) + " just dropped in!", description = "This poor soul fell through a portal onto a pile of lightly used mattresses while We were changing, and seemed unable to handle the psychic stress of our unfiltered form. They've passed out from shock for now, would someone make sure they still remember their name when they wake up? I believe it's " + str(verping) + ".", colour = embcol))

                    elif rand == 7:

                        await client.get_channel(828411760847356005).send(embed = discord.Embed(title = str(vername) + " could use some help", description = str(verping) + " seems to have had a recent encounter with some of the dungeon slimes. Could someone get them some clothes, and see to it that they are taken care of?", colour = embcol))

                    elif rand == 8:

                        await client.get_channel(828411760847356005).send(embed = discord.Embed(title = str(vername) + " has discovered a portal into the Dungeon!", description = "Oh Dear," + str(verping) + "appears to have been transported here from their native plane of existence! Could someone help them get settled into their new home?", colour = embcol))

                    elif rand == 9:

                        await client.get_channel(828411760847356005).send(embed = discord.Embed(title = "Helpfully discovering a hole in our ceiling, here's " + str(vername), description = "It's odd, We thought we had fixed that hole already? Could someone check if " + str(verping) + " is alright while we go see to the repairs again?", colour = embcol))

                    else:

                        await client.get_channel(828411760847356005).send(embed = discord.Embed(title = str(vername) + " has swung in!", description = "We think " + str(verping) + " must have had a run in with one of the amnesia blooms in the garden. They dont seem to remember where they are! Could someone help them get settled back in while We do some weeding?", colour = embcol))
            ...
            #Kink Functions
            ...
            elif message.content.lower().startswith(str(myprefix) + "kink"):
                ...
                if not str(targname) in str(kinkdata):
                    ...
                    await message.channel.send(embed = discord.Embed(title = "Could not find " + namestr.split("#")[0] + "'s kink list", description = "Make sure that <@" + str(targname.id) + "> has completed the (as yet unreleased) kink survey."))
                    ...
                    for a in range(len(kinkdata)):
                        ...
                        if a != 0:
                            ...
                            if str(targname) == str(kinkdata[a][1]):
                                ...
                                if message.content.lower().startswith(str(myprefix) + "kinklist"):
                                ...
                                #Kink checked random encounter

                                elif message.content.lower().startswith(str(myprefix) + "kinkencounter") and ("lorekeeper" in str(message.author.roles).lower() or message.author.name == "C_allum"):
            ...
            elif message.content.lower().startswith(str(myprefix) + "lottery") and "moderator" in str(authroles):

                pnames = sheet.values().get(spreadsheetId = SpreadsheetID, range = "B2:B1000").execute().get("values")

                plist = []

                for i in range(len(pnames)):

                    if not pnames[i][0] in str(plist):

                        plist.append(pnames[i][0])

                res = random.choice(plist)

                desc = "Out of " + str(len(plist)) + " players with characters submitted, " + res + "'s name was drawn"

                if " " in message.content:

                    msplt = message.content.split(" ",2)

                    quant = msplt[1]

                    desc += ", winning them the prize of " + str(quant) + ":dz:"

                    if len(msplt) > 2:

                        reason = msplt[2]

                        desc += "!\n\nThis lottery was drawn for : " + reason

                desc += "\n\nPlease note, this function allows one entry per *player*, regardless of the number of registered characters."

                print(message.author.name + " ran the lottery command. It was won by: " + res)

                await client.get_channel(logchannel).send(str(message.author) + " ran the lottery command. It was won by: " + res)
                
                await message.channel.send(embed = discord.Embed(title = res + " won!", description = desc, colour = embcol))

                await message.delete()

            #Potion Command

            elif message.content.lower().startswith(str(myprefix) + "potion"):

                poteffect = random.choice(["A", "B"])

                potemb = discord.Embed(title = message.author.name + " drank a potion of transformation!", description = poteffect, colour = embcol)

                await message.channel.send(embed = potemb)
